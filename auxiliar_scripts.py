...
class popupWindow(object):
    def __init__(self, master):
        ...
        self.label = tk.Label(top, text = 'Write a name for your selection')
        self.label.grid()
        self.entry = tk.Entry(top, textvariable=self.value)
        self.entry.grid()
        self.button = tk.Button(top, text = 'Ok', command = self.cleanup, state = 'disabled')
        self.button.grid()
        self.warning_label = tk.Label(top, text = 'This will be the name for the column where this field is stored.')
